  ...
  return (
    <div className="space-y-6 max-w-6xl">
      ...
      <div className="flex items-start justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <Link
            ...
            className="flex items-center gap-1.5 text-sm px-3 py-2 bg-[#21262d] hover:bg-[#30363d] text-slate-300 rounded-lg transition-colors"
          >
          ...
          {evento.spreadsheet_id && (
            <a
              ...
              className="flex items-center gap-1.5 text-sm px-3 py-2 bg-[#21262d] hover:bg-[#30363d] text-green-400 rounded-lg transition-colors"
            >
          ...
          <Link
            href={`/scan?evento=${id}`}
            className="flex items-center gap-1.5 text-sm px-3 py-2 bg-green-700 hover:bg-green-600 text-white rounded-lg transition-colors"
          >
      ...
      <div className="grid grid-cols-4 gap-4">
        <StatCard label="Fornecedores" value={fornecedores?.length ?? 0} icon={Users} color="text-purple-400" bg="bg-purple-500/10" />
        <StatCard label="Credenciados" value={totalFuncionarios} icon={UserCheck} color="text-blue-400" bg="bg-blue-500/10" />
        <StatCard label="Dentro agora" value={dentroAgora} icon={ScanLine} color="text-green-400" bg="bg-green-500/10" />
        <StatCard label="Entradas hoje" value={registros?.filter(r => r.tipo === 'entrada').length ?? 0} icon={Clock} color="text-orange-400" bg="bg-orange-500/10" />
      </div>
      ...
      <div className="grid grid-cols-5 gap-6">
        ...
        <div className="col-span-3 space-y-4">
          <div className="flex items-center justify-between">
            <h2 className="text-white font-semibold">Fornecedores</h2>
          ...
          {!fornecedores?.length ? (
            <div className="bg-[#161b22] border border-[#30363d] rounded-xl p-10 text-center">
              ...
              <p className="text-slate-500 text-sm">Nenhum fornecedor ainda</p>
            </div>
          ) : (
        ...
        {/* Feed de atividade do evento */}
        <div className="col-span-2 bg-[#161b22] border border-[#30363d] rounded-xl p-5">
          <h2 className="text-white font-semibold mb-4">Atividade do evento</h2>
          {!registros?.length ? (
            <p className="text-slate-500 text-sm text-center py-8">Nenhuma entrada/saída registrada</p>
          ) : (
              ...
              {registros.map((r) => {
                ...
                return (
                  <div key={r.created_at + r.funcionario_id} className="flex items-start gap-2.5">
                    <div className={`mt-1 w-1.5 h-1.5 rounded-full shrink-0 ${r.tipo === 'entrada' ? 'bg-green-400' : 'bg-yellow-400'}`} />
                    <div className="min-w-0">
                      <p className="text-white text-xs font-medium truncate">{func?.nome}</p>
                      <p className="text-slate-500 text-xs">{forn?.nome} • {func?.cargo}</p>
