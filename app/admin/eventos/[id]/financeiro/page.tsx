  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <StatCard label="Faturamento" value={brl(dados.faturamento)} icon={Wallet} tom="acento" />
        <StatCard
          label="Custos" value={brl(dados.custoTotal)}
          sub={`${dados.custos.length} lançamento${dados.custos.length === 1 ? '' : 's'}`}
          icon={Receipt} tom="aviso"
        />
        <StatCard
          label="Lucro" value={brl(dados.lucro)}
          sub={dados.margem !== null ? `${dados.margem.toFixed(1)}% de margem` : 'sem faturamento lançado'}
          icon={dados.lucro >= 0 ? TrendingUp : TrendingDown}
          tom={dados.lucro >= 0 ? 'sucesso' : 'erro'}
        />
      </div>

      <Secao tom="acento" icone={<Wallet className="w-3.5 h-3.5" />} titulo="Faturamento" descricao="A receita lançada deste evento, e a NFe" corpoClassName="p-5">
        <PainelFaturamento eventoId={id} faturamento={dados.faturamento} temNfe={dados.temNfe} nfeNome={dados.nfeNome} />
      </Secao>

      <Secao icone={<Receipt className="w-3.5 h-3.5" />} titulo="Custos" descricao="Cada gasto deste evento, por categoria" corpoClassName="p-4">
